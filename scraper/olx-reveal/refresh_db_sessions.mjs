...
for (const acc of names) {
  ...
  try {
    const ctx = await browser.newContext({ proxy: proxyForKey(keyFor(acc, salts[acc] || 0)), storageState: state, locale: 'pl-PL', timezoneId: 'Europe/Warsaw', viewport: { width: 1366, height: 900 } }); // przez proxy (residential) — Cognito blokuje refresh z datacenterowego IP
